  ...
  return (
    <Screen className="homeScreen">
      ...
      <div className="homeScene">
        ...
        <HomePenguin3D penguinData={homePenguin} />
      </div>
...
function HomePenguin3D({ penguinData }) {
  ...
  useEffect(() => {
    ...
    scene.add(keyLight);
    const fillLight = new THREE.PointLight(0x9feaff, 1.6, 8);
    fillLight.position.set(2.8, 1.8, 3.4);
    scene.add(fillLight);

    const darkMaterial = new THREE.MeshStandardMaterial({ color: 0x14253a, roughness: 0.52 });
    const whiteMaterial = new THREE.MeshStandardMaterial({ color: 0xfffbf4, roughness: 0.52 });
    const orangeMaterial = new THREE.MeshStandardMaterial({ color: 0xffaa3c, roughness: 0.42 });
    const cheekMaterial = new THREE.MeshStandardMaterial({ color: 0xffa9bc, emissive: 0xff6f99, emissiveIntensity: 0.1, roughness: 0.48 });
    const eyeMaterial = new THREE.MeshStandardMaterial({ color: 0x071423, roughness: 0.12 });
    const eyeHighlightMaterial = new THREE.MeshStandardMaterial({ color: 0xffffff, emissive: 0xffffff, emissiveIntensity: 0.26, roughness: 0.06 });
    const shadowMaterial = new THREE.MeshBasicMaterial({ color: 0x5c9fc2, transparent: true, opacity: 0.22 });

    const penguinGroup = new THREE.Group();
    const body = new THREE.Mesh(new THREE.SphereGeometry(0.76, 42, 30), darkMaterial);
    body.scale.set(0.9, 1.05, 0.72);
    body.position.set(0, -0.06, 0);
    const belly = new THREE.Mesh(new THREE.SphereGeometry(0.55, 36, 24), whiteMaterial);
    belly.scale.set(0.88, 1.02, 0.34);
    belly.position.set(0, -0.12, 0.48);
    const head = new THREE.Mesh(new THREE.SphereGeometry(0.58, 42, 28), darkMaterial);
    head.scale.set(1.04, 0.98, 0.92);
    head.position.set(0, 0.72, 0.08);
    const face = new THREE.Mesh(new THREE.SphereGeometry(0.43, 36, 20), whiteMaterial);
    face.scale.set(1.18, 0.88, 0.34);
    face.position.set(0, 0.7, 0.51);
    const beak = new THREE.Mesh(new THREE.ConeGeometry(0.13, 0.22, 24), orangeMaterial);
    ...
    for (const x of [-0.2, 0.2]) {
      const eye = new THREE.Mesh(new THREE.SphereGeometry(0.08, 22, 14), eyeMaterial);
      eye.scale.set(1, 1.16, 0.42);
      eye.position.set(x, 0.82, 0.84);
      const highlight = new THREE.Mesh(new THREE.SphereGeometry(0.025, 12, 8), eyeHighlightMaterial);
      highlight.position.set(x - 0.024, 0.86, 0.88);
      const cheek = new THREE.Mesh(new THREE.SphereGeometry(0.08, 16, 10), cheekMaterial);
      cheek.scale.set(1.35, 0.7, 0.22);
      cheek.position.set(x * 1.55, 0.55, 0.82);
      penguinGroup.add(eye, highlight, cheek);
      eyes.push(eye, highlight);
    }

    const flippers = [];
    for (const x of [-0.64, 0.64]) {
      const flipper = new THREE.Mesh(new THREE.SphereGeometry(0.18, 22, 12), darkMaterial);
      flipper.scale.set(0.46, 1.7, 0.36);
      flipper.position.set(x, 0.12, 0.04);
      flipper.rotation.z = x < 0 ? 0.48 : -0.48;
    ...
    for (const x of [-0.23, 0.23]) {
      const foot = new THREE.Mesh(new THREE.SphereGeometry(0.14, 18, 10), orangeMaterial);
      foot.scale.set(1.65, 0.5, 0.92);
      foot.position.set(x, -0.84, 0.54);
      foot.rotation.z = x < 0 ? 0.1 : -0.1;
    ...
    const shadow = new THREE.Mesh(new THREE.CircleGeometry(1.05, 48), shadowMaterial);
    shadow.rotation.x = -Math.PI / 2;
    shadow.position.set(0, -0.92, 0.28);
    scene.add(shadow);
    ...
    for (let i = 0; i < sparkleCount; i += 1) {
      sparklePositions[i * 3] = (Math.random() - 0.5) * 4.2;
      sparklePositions[i * 3 + 1] = Math.random() * 3.2 - 0.4;
      sparklePositions[i * 3 + 2] = Math.random() * 2.4 - 1.3;
    }
    sparkleGeometry.setAttribute("position", new THREE.BufferAttribute(sparklePositions, 3));
    const sparkles = new THREE.Points(sparkleGeometry, new THREE.PointsMaterial({ color: 0xffffff, size: 0.04, transparent: true, opacity: 0.72 }));
    scene.add(sparkles);

    const nameTag = document.createElement("div");
    nameTag.className = "homePenguin3dName";
    nameTag.textContent = penguinData?.name || "ペンギン";
    ...
    const animate = () => {
      ...
      const walk = Math.sin(t * 1.25);
      const step = Math.sin(t * 5.1);
      penguinGroup.position.x = walk * 0.62;
      penguinGroup.position.y = Math.abs(step) * 0.045;
      penguinGroup.rotation.y = -walk * 0.22;
      penguinGroup.rotation.z = Math.sin(t * 2.5) * 0.035;
      head.rotation.y = Math.sin(t * 1.8) * 0.12;
      head.rotation.z = Math.sin(t * 1.4) * 0.04;
      face.rotation.copy(head.rotation);
      beak.rotation.y = head.rotation.y;
      flippers[0].rotation.z = 0.5 + Math.sin(t * 5.1) * 0.22;
      flippers[1].rotation.z = -0.5 - Math.sin(t * 5.1) * 0.22;
      feet[0].position.y = -0.84 + Math.max(0, step) * 0.07;
      feet[1].position.y = -0.84 + Math.max(0, -step) * 0.07;
      eyes.forEach((eye) => {
        eye.scale.y = Math.sin(t * 1.35) > 0.985 ? 0.12 : 1.16;
      });
      shadow.scale.setScalar(1 + Math.abs(walk) * 0.04);
      sparkles.rotation.z = t * 0.035;
  ...
  return (
    <div className="homePenguin3d" ref={mountRef} aria-label={`${penguinData?.name || "ペンギン"}の3D表示`} />
  );
}
